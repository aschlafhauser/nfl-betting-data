...
if(!expertLedgerHealthy)failures.push('Primary NFL expert ledger is not losslessly synchronized with newer governed episode-audit records');
if(snapshotFailures.length)failures.push(`FINAL_SNAPSHOT_MISSING: ${snapshotFailures.map(x=>x.gameId||x.file).join(', ')}`);
...
console.log(`NFL weekly manifest ${status}: ${expected} games, ${deep.completedGameCount||0} Deep Dives, ${classIds.size} classified, ${snapshotFailures.length} final-snapshot failures.`);
