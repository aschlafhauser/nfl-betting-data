...
for(const g of games){for(const [side,name,abbr] of [['away',g.away,g.awayAbbr],['home',g.home,g.homeAbbr]]){const n=norm(abbr)||norm(name);if(!n)failures.push(`NFL_IDENTITY_UNRESOLVED_BOARD: ${g.gameId} ${side} ${name} / ${abbr}`);else{seenBoard.add(n);if(n!==String(abbr||'').toUpperCase())warnings.push(`NFL_IDENTITY_ALIAS_NORMALIZED: ${g.gameId} ${abbr} -> ${n}`)}}}
...
const boardPairs=new Map();for(const g of games){const a=norm(g.awayAbbr)||norm(g.away),h=norm(g.homeAbbr)||norm(g.home);if(a&&h)boardPairs.set(`${a}__${h}`,g.gameId)}
const matched=[];
...
const report={season:board.season||2026,week:board.week,checkedAt:new Date().toISOString(),registryVersion:registry.version,registryTeams:(registry.teams||[]).length,boardGames:games.length,espnEvents:espnRows.length,matchedGames:new Set(matched).size,warnings,failures,status:failures.length?'FAIL':warnings.length?'PASS-WITH-ALIASES':'PASS'};
await fs.writeFile('data/team-identity-audit-current.json',JSON.stringify(report,null,2)+'\n');
console.log(`NFL identity audit ${report.status}: board=${games.length}, ESPN=${espnRows.length}, matched=${report.matchedGames}, failures=${failures.length}, aliases=${warnings.length}`);if(warnings.length)console.warn(warnings.join('\n'));if(failures.length){console.error(failures.join('\n'));process.exit(1)}
